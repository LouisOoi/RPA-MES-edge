...
def migrate_v1_to_v2(doc: dict, *, updated_by: str) -> dict:
    ...
    for p in doc["points"]:
        ...
        if p["kind"] == "digital_out":
            # AR-01/AR-03: v2 requires owner + output_class on every edge-
            # writable output. v1 predates both fields entirely, so there is
            # no source data to migrate from. Defaulting to owner="edge",
            # output_class="indicator" is deliberately the most conservative
            # legal classification (matches the reference topology, which is
            # literally an LED) — NOT a claim that every migrated output is
            # safe to leave that way. Anything actually driving process
            # equipment must be reclassified by a human during review.
            point_v2["owner"] = "edge"
            point_v2["output_class"] = "indicator"
