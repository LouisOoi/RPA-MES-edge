...
**Per-account credentials, not shared logins (AR-05).** Each operator/admin
gets their own account provisioned by whoever deploys this app — there is
no factory-default shared password. Repeated failed logins against one
account lock it out for a cooldown period (a `423 account_locked` response
tells you how long); this is a brute-force defense, not a bug — wait it
out or have an admin re-provision the account rather than retrying faster.

**Transport (AR-05).** This app is a plain HTTP Flask process; it does not
terminate TLS itself. Production deployments must sit it behind a reverse
proxy that terminates TLS and either forwards over a private/loopback
connection or sets `X-Forwarded-Proto: https`. If the operator turns on
`require_tls=True` when constructing the app, every request except the
`/api/status` health check is rejected with `403 tls_required` unless it
arrived over a channel the app can confirm is secure. Only enable that
flag once the reverse proxy is actually in place — otherwise it silently
locks the UI out from behind, since nothing will look secure to it.
...
outputs aliasing the same coil, a rule pointing at a point you deleted, an
edge-writable output missing its `owner`/`output_class` (AR-01), or a rule
targeting a `plc`-owned output (AR-03) — you get a `422` back with a
`problems` list explaining exactly what and where, and NOTHING gets saved.
Fix and retry.

Every `digital_out` point you add needs `"owner": "edge"` and an
`"output_class"` of either `"indicator"` or `"non_safety_actuation"`. There
is deliberately no "safety" class available — anything whose failure could
injure someone or damage equipment does not belong on this rule engine at
all; wire it to a real PLC or safety controller instead, and mark that
point `"owner": "plc"` here (read-only, informational) if this terminal
still needs to see its state.

**Permit-to-edit gate (AR-07).** If your change adds or modifies a rule
that writes an `owner: "edge"` output, the PUT above comes back `409
permit_required` instead of applying — the body's `pending_output_states`
shows you exactly what each affected output's `safe_state` will be and
whether a rule will still drive it. Review that, then resend the SAME PUT
with `"permit_acknowledged": true` added at the top level of the request
body (not inside the config document) to apply it. Non-actuating changes
(new telemetry points, renames, scaling tweaks, rules that don't touch an
edge-owned output) never hit this gate — they apply on the first PUT.
