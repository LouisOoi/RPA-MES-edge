...
class TestWriteManager:
    ...
    def request_write(
        self, poll_engine, point_id: str, value: bool, *, confirm: bool, timeout_ms: int = 5000, now_ms: int,
    ) -> TestWriteResult:
        ...
        self._pending.append({
            "point": point_id, "revert_at_ms": now_ms + timeout_ms, "revert_value": revert_value,
        })
        return TestWriteResult(ok=True, message=f"will auto-revert to {revert_value} in {timeout_ms}ms")

    def apply_pending_reverts(self, poll_engine, now_ms: int) -> None:
        due, still_pending = [], []
