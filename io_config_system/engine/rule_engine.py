...
class RuleEngine:
    ...
    def evaluate_cycle(
        self, poll_engine, results: dict[str, ReadResult], *, now_ms: int, monotonic_ms: int | None = None,
    ) -> list[str]:
        """Runs pending pulse-reverts, then evaluates every enabled rule
        once. Returns the ids of rules whose `when` matched (then-branch
        fired) this cycle.

        AR-04: `monotonic_ms` (defaults to `now_ms` if not given, for
        callers/tests that only track one clock) is what pulse-revert
        scheduling uses — never `now_ms`, which is wall-clock and can step
        backward on an NTP correction. A relay pulse must revert on
        schedule regardless of what the wall clock just did."""
        if monotonic_ms is None:
            monotonic_ms = now_ms
        self._apply_pending_reverts(poll_engine, monotonic_ms)
        ...
        for rule in self.rules:
            ...
            if self._evaluate_when(rule, current_values, stale):
                self._run_actions(poll_engine, rule["id"], rule.get("then", []), monotonic_ms)
                fired.append(rule["id"])
            else:
                self._run_actions(poll_engine, rule["id"], rule.get("else", []), monotonic_ms)
