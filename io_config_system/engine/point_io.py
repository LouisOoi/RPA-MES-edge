...
def read_point(client, device: dict, point: dict) -> ReadResult:
    modbus = point["modbus"]
    fn = modbus["fn"]
    ...
    try:
        if fn == "read_coils":
            ...
            value = resp.bits[0] if modbus.get("count", 1) == 1 else list(resp.bits[: modbus["count"]])
        elif fn == "read_input_registers":
            resp = client.read_input_registers(modbus["address"], count=modbus.get("count", 1), device_id=device_id)
            if resp.isError():
                return ReadResult(value=None, stale=True, error=str(resp))
            value = resp.registers[0] if modbus.get("count", 1) == 1 else list(resp.registers)
            ...
            # write_coil points are outputs; there is nothing to poll-read.
            # The engine skips these on the read path (see poll_engine.py).
