...
### What this adds to the engineering plan (not yet built — scoped, not started)

1. **Multi-zone orchestrator** — runs N independent `PollEngine`/`RuleEngine`/`config_store` instances (the existing, tested Variant-A code, reused unmodified per zone) inside one Windows Service process, each supervised on its own thread so one zone's fault or wireless dropout doesn't affect the others.
2. **Windows-specific plumbing** — COM-port device paths instead of `/dev/ttyS*`; a Windows Service wrapper with its own watchdog/restart logic in place of systemd; a documented Windows Update/maintenance-window policy (Group Policy/registry) since we own the box.
3. **Zone-scoped Flask routes** — e.g. `/api/zone/<id>/io`, `/api/zone/<id>/live` — so one terminal's web UI can commission and monitor each zone independently rather than assuming a single global `io_config`.
4. **`link.medium` field** in the `io_config` schema per zone, plus the commissioning-UI selector that resets timeout/retry/poll to the medium's recommended defaults (already prototyped in the UI mockup — see below).
5. **New regression test**: a mixed-fleet run — one zone on wired-fast defaults and one on wireless-loose defaults — driven together inside the orchestrator, confirming the two independent engine instances' settings never bleed into each other.
6. **Commissioning checklist addition**: VLAN/isolation applies to Method A too, not just Method B; the existing Phase 6 checklist's "if wireless" branch needs a parallel "regardless of method" line for network isolation.
